...
class FlightControllerBridge(Node):
    def __init__(self):
        ...
        print("⭐ Flight Controller Bridge Active ⭐")
    ...
    def joystick_callback(self, msg):
        ...
        print(f"\033[K\rReceived message #{self.msg_count}: x={msg.x:.2f}, y={msg.y:.2f}, z={msg.z:.2f} → throttle={self.controls['throttle']:.2f}, pitch={self.controls['pitch']:.2f}, roll={self.controls['roll']:.2f}", end="")
    ...
    def command_loop(self):
        ...
        while self.running and rclpy.ok():
            try:
                ...
                # Print command status periodically (once per second)
                current_time = time.time()
                if current_time - last_sent_time >= 1.0:
                    last_sent_time = current_time
                    print(f"\n\033[90mSent MSP command to FC: throttle={self.controls['throttle']:.2f}, pitch={self.controls['pitch']:.2f}, roll={self.controls['roll']:.2f}\033[0m")
                ...
                print(f"\033[31mError sending command to flight controller: {e}\033[0m")
    ...
    def destroy_node(self):
        ...
        if hasattr(self, 'ser') and self.ser.is_open:
            self.ser.close()
        print("\n\033[1;33mShutting down Flight Controller Bridge...\033[0m")
        super().destroy_node()
